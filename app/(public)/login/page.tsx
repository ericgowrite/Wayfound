...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(msg.includes("invalid-credential") || msg.includes("wrong-password") || msg.includes("user-not-found") ? "Invalid email or password" : msg);
    } finally {
      setLoading(false);
    }
  }
    ...
    <div style={{ minHeight: "100vh", background: "#FAF8F5", display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 16px", fontFamily: "var(--font-encode), ui-sans-serif, system-ui, sans-serif" }}>
      <div style={{ width: "100%", maxWidth: 480, background: "#FAF8F5", border: "1px solid #E8E8E8", borderRadius: 12, padding: "56px 48px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
        ...
        <button
          ...
          style={{ border: "1px solid #E8E8E8", borderRadius: 4, padding: "13px 16px", fontSize: 15, color: "#1A1A1A", fontWeight: 600, width: "100%", marginTop: 24, display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: "#FFFFFF", cursor: "pointer" }}
        >
          <GoogleIcon /><span>Continue with Google</span>
        </button>

        <div style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", marginTop: 24 }}>
        ...
        <form onSubmit={handleSubmit} style={{ width: "100%", maxWidth: 320 }}>
          ...
          <button
            ...
            style={{ background: "#2C3E50", color: "#fff", textAlign: "center", fontSize: 15, fontWeight: 600, padding: 14, borderRadius: 999, marginTop: 16, width: "100%", border: "none", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.6 : 1 }}
          >
            {loading ? "Signing in…" : "Sign in →"}
          </button>
        </form>

        <div style={{ fontSize: 13, color: "#3D5A73", marginTop: 18 }}>
          New here?{" "}
